...
# %% auto 0
__all__ = ['franken_class_map', 'TextT', 'TextPresetsT', 'CodeSpan', 'Blockquote', 'CodeBlock', 'H1', 'H2', 'H3', 'H4', 'ButtonT',
           'Button', 'ContainerT', 'BackgroundT', 'Container', 'Titled', 'DividerT', 'Divider', 'DividerSplit',
           'DividerLine', 'Article', 'ArticleTitle', 'ArticleMeta', 'SectionT', 'Section', 'Form', 'Fieldset', 'Legend',
           'Input', 'Select', 'Radio', 'CheckboxX', 'Range', 'TextArea', 'Switch', 'FormLabel', 'LabelT', 'Label',
           'UkFormSection', 'GenericLabelInput', 'LabelInput', 'LabelRange', 'LabelTextArea', 'LabelSwitch',
           'LabelRadio', 'LabelCheckboxX', 'LabelSelect', 'Options', 'UkSelect', 'LabelUkSelect', 'AT', 'ListT',
...
class TextPresetsT(VEnum):
    'Common Typography Presets'
    caption = stringify((TextT.muted, TextT.sm))  
    meta_text = stringify((TextT.sm,TextT.muted))
    helper_text = stringify((TextT.muted, TextT.sm, TextT.italic))
    label = stringify((TextT.medium, TextT.sm))  
    quote = stringify((TextT.italic, TextT.lg))  
    
    info = stringify(   (TextT.info,    TextT.sm, TextT.medium))
    success = stringify((TextT.success, TextT.sm, TextT.medium)) 
    warning = stringify((TextT.warning, TextT.sm, TextT.bold))
    error = stringify(  (TextT.error,   TextT.sm, TextT.bold))  
    
    subtitle = stringify((TextT.muted, TextT.lg, TextT.medium))
    subheading = stringify((TextT.bold, TextT.sm))

    emphasis = stringify((TextT.sm, TextT.medium)) 
    primary = stringify((TextT.primary,TextT.medium, TextT.muted))
...
def UkFormSection(title, description, *c, button_txt='Update', outer_margin=6, inner_margin=6):
    ...
    return Div(cls=f'space-y-{inner_margin} py-{outer_margin}')(
        Div(H3(title), P(description, cls=TextPresetsT.caption)),
        DividerSplit(), *c,
...
def NavSubtitle(*c, # Components
                 cls=TextPresetsT.caption, # Additional classes on the div
                 **kwargs # Additional args for the div
...
def NavBarSubtitle(title, # Title
                   subtitle, # Subtitle
                   cls=(), # Additional classes on the div
                   subtitle_cls=TextPresetsT.caption, # Additional classes on the subtitle
                   **kwargs # Additional args for the div
                  )->FT: # NavBar subtitle
