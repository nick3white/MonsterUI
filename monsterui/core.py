...
# %% auto 0
__all__ = ['HEADER_URLS', 'daisy_styles', 'fast_app', 'FastHTML', 'init_scrollspy', 'Theme']
...
# %% ../nbs/01_core.ipynb
def init_scrollspy(app):
    """Initializes Scrollspy with the app instance."""
    if not app:
        raise ValueError("App instance is required")
    app.hdrs += [
        Script("""
        document.addEventListener('DOMContentLoaded', function() {
            const spyElements = document.querySelectorAll('[data-scrollspy]');
            const observer = new IntersectionObserver(entries => {
                entries.forEach(entry => {
                    const target = document.querySelector(`[href="#${entry.target.id}"]`);
            ...
            spyElements.forEach(element => observer.observe(element));
        });
        """)
    ]
