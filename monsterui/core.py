...
def fast_app(*args, pico=False, **kwargs):
    ...
    if "bodykw" not in kwargs:
        kwargs["bodykw"] = {}
    if "class" not in kwargs["bodykw"]:
        kwargs["bodykw"]["class"] = ""
    kwargs["bodykw"]["class"] = stringify(
        (kwargs["bodykw"]["class"], "bg-background text-foreground")
    )
...
class Theme(Enum):
    ...
    """),
            _headers_theme(
                self.value, mode=mode, radii=radii, shadows=shadows, font=font
            ),
            scrollspy_style,
        ]
        ...
        if apex_charts:
            hdrs += [fh.Script(type="module", src=urls["apex_charts"])]
        ...
        if highlightjs:
            hdrs += [
                ...
                fh.Link(
                    rel="stylesheet", href=urls["highlight_light_css"], id="hljs-light"
                ),
                fh.Link(
                    rel="stylesheet", href=urls["highlight_dark_css"], id="hljs-dark"
                ),
    ...
    def local_headers(
        ...
        local_urls = dict(
            [_download_resource(url, static_dir) for url in HEADER_URLS.items()]
        )
        return self._create_headers(
            local_urls,
            mode=mode,
