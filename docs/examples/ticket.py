...
def TicketCard(id, title, description, status, step, department):
    return Card(
        ...
        CardBody(
            ...
            DividerSplit(cls="my-6"), 
            DivFullySpaced(
                Div(P("Department", cls=TextPresetsT.caption),
                    P(department, cls=TextPresetsT.emphasis),
                    cls='space-y-3'),
                Div(P("Last Updated", cls=TextPresetsT.caption),
                    P(datetime.now().strftime('%b %d, %H:%M'), cls=TextPresetsT.emphasis),
                    cls='space-y-3'),
