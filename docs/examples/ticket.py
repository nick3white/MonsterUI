...
def TicketCard(id, title, description, status, step, department):
    return Card(
        ...
        CardBody(
            ...
            DividerSplit(cls="my-6"), 
            DivFullySpaced(
                Div(P("Department", cls=TextFont.muted_sm),
                    P(department, cls=TextFont.md_weight_sm),
                    cls='space-y-3'),
                Div(P("Last Updated", cls=TextFont.muted_sm),
                    P(datetime.now().strftime('%b %d, %H:%M'), cls=TextFont.md_weight_sm),
                    cls='space-y-3'),
